...
(function () {
  ...
  const rememberedView=read('bedo-last-view',{}),pathPage=location.pathname.slice(1);
  let page = ['brainstorm','insights','settings'].includes(pathPage) ? pathPage : (!demo&&['calendar','brainstorm','insights','settings'].includes(rememberedView.page)?rememberedView.page:'calendar');
  let view = !demo&&['month','day','week','checklist'].includes(rememberedView.view)?rememberedView.view:'month', selected = !demo&&rememberedView.date?fromKey(rememberedView.date):new Date(), month = !demo&&rememberedView.date?fromKey(rememberedView.date):new Date(), filter = 'All', drag = null;
  let settingsSection='profile';
  let insightsExpanded=false,insightPeriod='week';const revealedNotes=new Set();
  ...
  function reconcileBlockStatuses(){
    ...
    blocks=blocks.map(block=>{
      if(isBedoBlock(block)&&blockStatus(block)==='planned'&&blockEnd(block)<=now){changed=true;return {...block,status:'unconfirmed',completed:false};}
      return block;
    });
    if(changed)save();
  }
  ...
  function dayCompleted(day) {
    const all = blocks.filter(b => b.date === dateKey(day) && isBedoBlock(b));
    return all.length > 0 && all.every(b => blockStatus(b) === 'done');
  }
  ...
  function render() {
    ...
    root.querySelectorAll('[data-block]').forEach(element=>{const block=blocks.find(item=>item.id===element.dataset.block);if(!block?.locked)return;element.classList.add('locked');const handle=element.querySelector('[data-drag]');if(handle){handle.disabled=true;handle.setAttribute('aria-label',block.title+' is locked');handle.textContent='🔒';}const resize=element.querySelector('[data-resize]');if(resize){resize.disabled=true;resize.setAttribute('aria-label',block.title+' is locked');}});
    if(page==='calendar'){
      const header=root.querySelector('.bd-top');header.classList.add('bd-calendar-header');
      root.querySelector('.bd-heading')?.insertAdjacentHTML('afterend',realityPrompt());
      ...
      const shareArea=document.createElement('section');shareArea.className='bd-calendar-share';shareArea.innerHTML='<h2>Share your schedule</h2><p>Send a text rundown, or publish selected days as a live read-only BEDO view.</p><div><button class="bd-secondary" data-action="share-day">Share today as text</button><button class="bd-primary" data-action="share">Share selected days live</button></div>';root.querySelector('.bd-content').append(shareArea);
      if(['day','week'].includes(view)){layoutTimelineOverlaps();requestAnimationFrame(layoutTimelineOverlaps);}
    }
    ...
    if(!demo)localStorage.setItem('bedo-last-view',JSON.stringify({page,view,date:dateKey(selected)}));
  }
  ...
  function organizeSettings(){
    ...
    const recoveryButton=document.querySelector('[data-action=recovery]');if(recoveryButton){recoveryButton.textContent='Download latest autosave';const restore=document.createElement('button');restore.className='bd-secondary';restore.dataset.action='restore-autosave';restore.textContent='Restore latest autosave';recoveryButton.after(restore);}
  ...
  function calendar() {
    const title = view==='month' ? 'A little space for everything.' : view==='checklist'?'This week, one clear list.':dateLabel(selected,{weekday:'long',month:'long',day:'numeric'}),weekView=['week','checklist'].includes(view);
    return `<section class="bd-heading"><div><span class="bd-kicker">YOUR TIME, YOUR PACE ✨</span><h1>${title}</h1><p>${view==='month'?'Pick a day. Make room for what matters.':view==='checklist'?'Priorities first. Tick things off in one easy scroll.':'A flexible plan. Not a perfect one.'}</p></div></section><div class="bd-toolbar"><div class="bd-segments">${['month','day','week','checklist'].map(v=>`<button data-view="${v}" class="${view===v?'active':''}">${v==='month'?'Calendar':v[0].toUpperCase()+v.slice(1)}</button>`).join('')}</div><div class="bd-date-nav"><button data-action="prev" aria-label="Previous period">‹</button><strong>${view==='month'?dateLabel(month,{month:'long',year:'numeric'}):weekView?'Week of '+dateLabel(monday(),{month:'short',day:'numeric'}):dateLabel(selected,{month:'short',day:'numeric'})}</strong><button data-action="next" aria-label="Next period">›</button><button data-action="today">Today</button></div></div><p class="bd-view-swipe-hint" aria-label="Navigation tip">↔ Swipe to switch Calendar, Day, Week, and Checklist</p>${filters()}${view==='month'?monthStream():view==='week'?weekCalendar():view==='checklist'?checklist():dayCalendar(selected)}<p class="bd-help">${view==='month'?'Tap any date to explore and plan your day.':view==='checklist'?'High-priority blocks appear first. Tick any item when it is done.':'Drag the ⋮⋮ handle to move a block. In Week view, drag sideways to move it to another day.'}</p>`;
  }
  function realityPrompt(){
    const unresolved=blocks.filter(item=>isBedoBlock(item)&&blockStatus(item)==='unconfirmed').sort((a,b)=>dayNumber(a.date)-dayNumber(b.date)||a.start-b.start),upcoming=blocks.filter(item=>isBedoBlock(item)&&blockStatus(item)==='planned').sort((a,b)=>blockEnd(a)-blockEnd(b)),block=unresolved[0]||upcoming[0];
    if(!block)return '';
    const needsAnswer=blockStatus(block)==='unconfirmed';
    return `<section class="bd-reality-prompt ${needsAnswer?'needs-answer':'is-upcoming'}"><div><span>UP NEXT</span><strong>${esc(block.title)}</strong><p><b>${dateLabel(fromKey(block.date),{weekday:'long',month:'short',day:'numeric'})}</b><b>${clockTime(block.start)}–${clockTime(Number(block.start)+Number(block.duration))}</b><em>${needsAnswer?'Awaiting your update':statusLabel(blockStatus(block))}</em></p></div>${needsAnswer?`<div aria-label="Choose what happened"><button class="bd-primary" data-reality="done" data-id="${esc(block.id)}">Done</button><button class="bd-secondary" data-reality="move" data-id="${esc(block.id)}">Move</button><button class="bd-secondary" data-reality="skipped" data-id="${esc(block.id)}">Skip</button></div>`:''}</section>`;
  }
  ...
  function checklist(){
    const start=monday(),end=new Date(start);end.setDate(end.getDate()+6);const weight={high:0,normal:1,low:2},rank={unconfirmed:0,planned:1,external:1,skipped:2,moved:2,done:3};
    const items=blocks.filter(block=>((blockStatus(block)==='unconfirmed'&&isBedoBlock(block))||(dayNumber(block.date)>=dayNumber(dateKey(start))&&dayNumber(block.date)<=dayNumber(dateKey(end))))&&(filter==='All'||category(block)===filter)).sort((a,b)=>(rank[blockStatus(a)]??1)-(rank[blockStatus(b)]??1)||(weight[a.priority||'normal']-weight[b.priority||'normal'])||dayNumber(a.date)-dayNumber(b.date)||a.start-b.start);
    return `<div class="bd-checklist">${items.map(block=>{const status=blockStatus(block),done=status==='done';return `<article class="${done?'done':''} status-${status}">${isBedoBlock(block)?`<button data-complete="${esc(block.id)}" aria-label="${done?'Mark planned':'Mark done'} ${esc(block.title)}">${done?'✓':'○'}</button>`:'<span class="bd-check-placeholder">↗</span>'}<div><span>${dateLabel(fromKey(block.date),{weekday:'short',month:'short',day:'numeric'})} · ${clockTime(block.start)}</span><strong>${esc(block.title)}</strong><small>${esc(category(block))} · ${statusLabel(status)}</small></div><em class="${esc(block.priority||'normal')}">${block.priority==='high'?'High priority':block.priority==='low'?'Low priority':'Normal'}</em><button data-edit="${esc(block.id)}" aria-label="Edit ${esc(block.title)}">Edit</button></article>`;}).join('')||'<div class="bd-empty">Nothing planned this week yet.</div>'}</div>`;
  }
  function layoutTimelineOverlaps(){root.querySelectorAll('.bd-day-track').forEach(track=>{const elements=[...track.querySelectorAll('.bd-block')],items=elements.map(element=>({element,block:blocks.find(item=>item.id===element.dataset.block)})).filter(item=>item.block).sort((a,b)=>a.block.start-b.block.start),groups=[];let group=[],end=-1;for(const item of items){const start=Number(item.block.start),finish=start+Number(item.block.duration);if(group.length&&start>=end){groups.push(group);group=[];end=-1;}group.push(item);end=Math.max(end,finish);}if(group.length)groups.push(group);for(const cluster of groups){const slots=[];for(const item of cluster){const start=Number(item.block.start);let slot=slots.findIndex(until=>until<=start);if(slot<0)slot=slots.length;slots[slot]=start+Number(item.block.duration);item.slot=slot;}const count=Math.max(1,slots.length);for(const item of cluster){item.element.style.left=`calc(${item.slot/count*100}% + 4px)`;item.element.style.width=`calc(${100/count}% - 8px)`;item.element.style.right='auto';item.element.style.zIndex=String(item.slot+1);}}});}
  function dayCalendar(day, compact=false) {
    const list=visible(day), height=compact?50:72;
    return `<div class="bd-timeline ${compact?'compact':''}" style="--hour:${height}px"><div class="bd-time-axis">${Array.from({length:24},(_,h)=>`<span>${compact?String(h).padStart(2,'0'):clockTime(h)}</span>`).join('')}</div><div class="bd-day-track" data-track-date="${dateKey(day)}" data-hour-height="${height}">${list.map(b=>{const status=blockStatus(b),done=status==='done';return `<article class="bd-block ${tint(category(b))} ${done?'done':''} status-${status}" data-block="${esc(b.id)}" style="top:${Number(b.start)*height}px;height:${Math.max(42,Number(b.duration)*height-3)}px;${blockColorStyle(category(b))}"><button class="bd-drag" data-drag="${esc(b.id)}" aria-label="Drag ${esc(b.title)} to a new time">⋮⋮</button><button class="bd-block-body" data-edit="${esc(b.id)}"><strong>${esc(b.title)}</strong><small>${clockTime(b.start)} – ${clockTime(Number(b.start)+Number(b.duration))} · ${Math.round(b.duration*100)/100}h</small><em>${esc(category(b))} · ${statusLabel(status)}</em></button>${isBedoBlock(b)?`<button class="bd-check" data-complete="${esc(b.id)}" aria-label="${done?'Mark planned':'Mark done'} ${esc(b.title)}">${done?'✓':'○'}</button>`:'<span class="bd-external-mark">↗</span>'}${compact?'':`<button class="bd-resize" data-resize="${esc(b.id)}" aria-label="Resize ${esc(b.title)}">↕</button>`}</article>`;}).join('')}${dateKey(day)===dateKey(new Date())?`<div class="bd-now" style="top:${(new Date().getHours()+new Date().getMinutes()/60)*height}px"></div>`:''}</div></div>${!list.length&&!compact?'<div class="bd-empty">A little breathing room 🌱<p>Add a block to shape this day.</p></div>':''}`;
  }
  function brainstorm() { return `<section class="bd-heading"><div><span class="bd-kicker">LESS IN YOUR HEAD 🧠</span><h1>Room for your thoughts.</h1><p>Capture the messy middle. Give an idea a time when you’re ready.</p></div></section><div class="bd-note-grid">${ideas.map(n=>{const hidden=n.private&&!revealedNotes.has(n.id);return `<article class="bd-note ${hidden?'bd-note-private':''}"><span class="bd-note-label">${n.private?'🔒 PRIVATE THOUGHT':'A LITTLE THOUGHT'}</span><p>${hidden?'This note is hidden.':esc(n.text)}</p><div>${hidden?`<button data-reveal-note="${esc(n.id)}">Reveal</button>`:`<button data-schedule-note="${esc(n.id)}">↗ Add to schedule</button><button data-edit-note="${esc(n.id)}" aria-label="Edit note">Edit</button>`}<button data-delete-note="${esc(n.id)}" aria-label="Delete note">×</button></div></article>`;}).join('')||'<div class="bd-empty">Start with a thought ✍️<p>No sorting required. This is your space.</p></div>'}</div>`; }
  function insights() {
    const start=monday(),days=Array.from({length:7},(_,i)=>{const d=new Date(start);d.setDate(d.getDate()+i);return d;});
    const tracked=blocks.filter(isBedoBlock),week=tracked.filter(b=>days.some(d=>b.date===dateKey(d))),done=week.filter(b=>blockStatus(b)==='done'),hours=done.reduce((sum,b)=>sum+Number(b.duration||0),0);
    const now=selected,periodBlocks=tracked.filter(b=>{const d=fromKey(b.date);if(insightPeriod==='week')return d>=start&&d<=days[6];if(insightPeriod==='month')return d.getFullYear()===now.getFullYear()&&d.getMonth()===now.getMonth();return d.getFullYear()===now.getFullYear();}),periodDone=periodBlocks.filter(b=>blockStatus(b)==='done'),periodHours=periodDone.reduce((sum,b)=>sum+Number(b.duration||0),0),plannedHours=periodBlocks.reduce((sum,b)=>sum+Number(b.duration||0),0),rate=plannedHours?Math.round(periodHours/plannedHours*100):0,achievement=rate>=90?'🏆 Remarkable rhythm':rate>=70?'🎯 Strong momentum':rate>=40?'🌱 Building consistency':'✨ Every step counts';
    const series=insightChartSeries(tracked,start,days,now),max=Math.max(1,...series.map(item=>item.planned));
    return `<section class="bd-heading"><div><span class="bd-kicker">PROGRESS, NOT PRESSURE 🌻</span><h1>Your week, in motion.</h1><p>${dateLabel(start,{month:'short',day:'numeric'})} – ${dateLabel(days[6],{month:'short',day:'numeric'})}. Small steps add up.</p></div></section><div class="bd-stats"><article><span>Completed blocks</span><strong>${done.length}<small> / ${week.length}</small></strong></article><article><span>Weekly follow-through</span><strong>${week.reduce((s,b)=>s+Number(b.duration||0),0)?Math.round(hours/week.reduce((s,b)=>s+Number(b.duration||0),0)*100):0}%</strong></article><article><span>Completed time</span><strong>${Math.round(hours*100)/100}<small>h</small></strong></article></div><section class="bd-card"><h2>Weekly motion</h2><p>Completed blocks across your week.</p><div class="bd-bars">${days.map(d=>{const all=tracked.filter(b=>b.date===dateKey(d)),complete=all.filter(b=>blockStatus(b)==='done').length;return `<div><small>${complete}/${all.length}</small><div class="bd-bar" style="height:${all.length?Math.max(5,complete/all.length*140):5}px"></div><span>${dateLabel(d,{weekday:'short'})}</span></div>`;}).join('')}</div></section><section class="bd-card"><h2>Space for different parts of life</h2>${categories.map(c=>`<div class="bd-category-stat"><span><i class="${tint(c)}" style="background:${categoryColor(c)}"></i>${esc(c)}</span><strong>${week.filter(b=>category(b)===c&&blockStatus(b)==='done').length} completed</strong></div>`).join('')}</section><section class="bd-card bd-long-insights"><h2>Achievements over time</h2><p>Open this only when you want the bigger picture.</p><button class="bd-secondary" data-action="insights-toggle" aria-expanded="${insightsExpanded}">${insightsExpanded?'Hide extended stats':'View weekly, monthly & yearly stats'}</button>${insightsExpanded?`<div class="bd-segments bd-insight-periods">${['week','month','year'].map(period=>`<button data-insight-period="${period}" class="${insightPeriod===period?'active':''}">${period[0].toUpperCase()+period.slice(1)}</button>`).join('')}</div><div class="bd-achievement"><strong>${achievement}</strong><span>${Math.round(periodHours*100)/100}h completed of ${Math.round(plannedHours*100)/100}h planned · ${rate}% follow-through</span></div><div class="bd-achievement-chart" role="img" aria-label="Planned and completed time chart">${series.map(item=>`<div><div class="bd-chart-bars"><i class="planned" style="height:${Math.max(3,item.planned/max*120)}px" title="${item.planned} planned hours"></i><i class="done" style="height:${Math.max(2,item.done/max*120)}px" title="${item.done} completed hours"></i></div><span>${item.label}</span></div>`).join('')}</div><div class="bd-chart-legend"><span><i class="planned"></i>Planned</span><span><i class="done"></i>Done</span></div><div class="bd-reality-stats">${['done','skipped','moved','unconfirmed'].map(status=>`<span><strong>${periodBlocks.filter(b=>blockStatus(b)===status).length}</strong>${statusLabel(status)}</span>`).join('')}</div>`:''}</section>`;
  }
  function insightChartSeries(tracked,start,days,now){
    let buckets;
    if(insightPeriod==='week')buckets=days.map(day=>({label:dateLabel(day,{weekday:'narrow'}),test:block=>block.date===dateKey(day)}));
    else if(insightPeriod==='month')buckets=Array.from({length:Math.ceil(new Date(now.getFullYear(),now.getMonth()+1,0).getDate()/7)},(_,index)=>({label:'W'+(index+1),test:block=>{const day=fromKey(block.date);return day.getFullYear()===now.getFullYear()&&day.getMonth()===now.getMonth()&&Math.floor((day.getDate()-1)/7)===index;}}));
    else buckets=Array.from({length:12},(_,index)=>({label:new Date(2000,index,1).toLocaleDateString(undefined,{month:'narrow'}),test:block=>{const day=fromKey(block.date);return day.getFullYear()===now.getFullYear()&&day.getMonth()===index;}}));
    return buckets.map(bucket=>{const list=tracked.filter(bucket.test);return {label:bucket.label,planned:Math.round(list.reduce((sum,b)=>sum+Number(b.duration||0),0)*10)/10,done:Math.round(list.filter(b=>blockStatus(b)==='done').reduce((sum,b)=>sum+Number(b.duration||0),0)*10)/10};});
  }
  function appGuide() {
    return `<section class="bd-card bd-guide" id="bd-app-guide"><h2>How to use app</h2><p>A short, guided walk through planning, moving blocks, Brainstorm, Insights, and saving. It opens each feature and shows you where to look. Nothing in your schedule is changed.</p><button class="bd-primary" data-action="tour">Start app tour →</button><p>You can replay it here whenever you like.</p><div class="bd-guide-tips"><h2>Tips on timeblocking</h2><button class="bd-secondary" data-action="tips-toggle" aria-expanded="false">Show 3 tips</button><div data-tips-panel hidden>${timeblockingTips()}</div></div></section>`;
  }
  function timeblockingTips(){return `<div class="bd-tips-grid"><article><span>BEDO’S RHYTHM</span><h3>Plan, adjust, notice.</h3><p>Plan the week before it starts. Adjust tomorrow the night before. Check blocks at midday and before bed—not for perfection, but awareness.</p></article><article><span>MAKE THE WHEN SPECIFIC</span><h3>Give intentions a time.</h3><p>Decide when and how you will act. A specific plan makes an intention easier to begin.</p><small>Gollwitzer & Sheeran · Implementation Intentions research</small></article><article><span>PLAN FOR REAL LIFE</span><h3>Leave breathing room.</h3><p>Expect interruptions and keep some buffer. A useful schedule can bend when the day changes.</p><small>Parke et al. · Journal of Applied Psychology</small></article></div>`;}
  function savingCard() {
    return `<section class="bd-card" id="bd-saving-card"><h2>Your saved data</h2><p id="bd-save-status" role="status"></p><p id="bd-save-time"></p><p>Every change is autosaved on this device with rolling recovery copies. Connect Google Drive on each phone or computer to sync the same private BEDO file between them. BEDO cannot browse your other Drive files.</p><button class="bd-secondary" data-action="sync-now">Save online now</button><button class="bd-secondary" data-action="export">Download my data</button><button class="bd-secondary" data-action="recovery">Download previous device copy</button></section>`;
  }
  ...
  function downloadData(recovery=false){
    const account=window.BedoAuth?.currentUser(),autosave=read('bedo-autosave-backups',[])[0]?.data,snapshot=recovery?(autosave||read('bedo-recovery-'+account?.sub,null)):window.BedoSync?.exportData()||{blocks,ideas,profile};
  ...
  async function publishShare(disable=false) {
    ...
    try{
      ...
      status.innerHTML=`<a href="${esc(url)}" target="_blank" rel="noopener">Open live schedule ↗</a><div><input id="bd-live-link" readonly aria-label="Live schedule link" value="${esc(url)}"><button class="bd-secondary" data-action="copy-live">Copy link</button></div>`;
  ...
  function blockDialog(id, text='') {
    const b=blocks.find(item=>item.id===id),day=b?.date||dateKey(selected),templates=read('bedo-block-templates',[]),blockDay=fromKey(day),weekdays=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
    dialog(`<h2>${b?'Edit this block':'Make a little room.'}</h2><p>${b?'Choose whether this change applies once or to the whole recurring series.':'Give something important a place in your day.'}</p><form id="bd-block-form" data-id="${esc(id||'')}">${!b&&templates.length?`<label>Use a saved block<select name="template"><option value="">Start from scratch</option>${templates.map((t,i)=>`<option value="${i}">${esc(t.title)}</option>`).join('')}</select></label>`:''}<label>What’s the plan?<input name="title" required maxlength="120" value="${esc(b?.title||text)}" placeholder="A little deep work"></label><label>Day<input name="date" type="date" required value="${inputDate(day)}"></label><div class="bd-form-row"><label>Start time<input name="start" type="time" step="300" required value="${time(b?.start??9)}"></label><label>Set the length by<select name="lengthMode"><option value="minutes">Minutes</option><option value="hours">Hours</option><option value="end">End time</option></select></label></div><label data-length-field="minutes">Minutes<input name="minutes" type="number" min="5" max="1440" step="5" value="${Math.round((b?.duration??1)*60)}"></label><label data-length-field="hours" hidden>Hours<input name="hours" type="number" min="0.08" max="24" step="0.25" value="${Math.round((b?.duration??1)*100)/100}"></label><label data-length-field="end" hidden>End time<input name="end" type="time" step="300" value="${time((b?.start??9)+(b?.duration??1))}"></label><label>Category<select name="category">${[...new Set([...categories,category(b||{})])].map(c=>`<option ${category(b||{})===c?'selected':''}>${esc(c)}</option>`).join('')}</select></label>${b?'':`<label>Repeat<select name="repeat"><option value="none">Does not repeat</option><option value="daily">Daily · next 4 weeks</option><option value="weekdays">Monday–Friday · next 12 weeks</option><option value="custom-days">Choose weekdays · next 12 weeks</option><option value="weekly">Weekly · next 12 weeks</option><option value="monthly">Monthly · next 12 months</option><option value="yearly">Yearly · choose months</option></select></label><fieldset class="bd-repeat-weekdays" hidden><legend>Repeat on</legend>${weekdays.map((name,i)=>`<label><input type="checkbox" name="repeatWeekday" value="${i}" ${i===blockDay.getDay()?'checked':''}>${name}</label>`).join('')}</fieldset><fieldset class="bd-repeat-months" hidden><legend>Repeat in these months</legend>${Array.from({length:12},(_,i)=>`<label><input type="checkbox" name="repeatMonth" value="${i}" ${i===blockDay.getMonth()?'checked':''}>${new Date(2000,i,1).toLocaleDateString(undefined,{month:'short'})}</label>`).join('')}</fieldset>`}<label class="bd-check-label"><input type="checkbox" name="locked" ${b?.locked?'checked':''}> Lock this block’s time</label>${b?.seriesId?'<label class="bd-check-label"><input type="checkbox" name="applySeries"> Apply these changes to all future blocks in this series</label>':''}<label class="bd-check-label"><input type="checkbox" name="saveTemplate"> Save this block for future use</label><div class="bd-form-actions">${b?`<button type="button" class="bd-danger" data-delete-block="${esc(id)}">Delete this block</button>`:''}<button class="bd-primary">Save block</button></div><p class="bd-form-error" role="alert"></p></form>`);
    const form=document.getElementById('bd-block-form'),showLength=()=>form.querySelectorAll('[data-length-field]').forEach(field=>field.hidden=field.dataset.lengthField!==form.elements.lengthMode.value),currentStatus=blockStatus(b||{});form.elements.category.closest('label').insertAdjacentHTML('afterend',`${b?.source==='google-calendar'?'<p class="bd-imported-note">Imported from Google Calendar. BEDO completion tracking stays separate.</p>':`<label>Status<select name="status">${['planned','done','skipped','moved','unconfirmed'].map(status=>`<option value="${status}" ${currentStatus===status?'selected':''}>${statusLabel(status)}</option>`).join('')}</select></label>`}<label>Priority<select name="priority"><option value="normal" ${(b?.priority||'normal')==='normal'?'selected':''}>Normal</option><option value="high" ${b?.priority==='high'?'selected':''}>High</option><option value="low" ${b?.priority==='low'?'selected':''}>Low</option></select></label>`);form.elements.hours.min='0';form.elements.lengthMode.addEventListener('change',showLength);showLength();
    if(b){form.querySelector('.bd-form-actions').insertAdjacentHTML('afterbegin',`<button type="button" class="bd-secondary" data-duplicate-block="${esc(b.id)}">Duplicate</button>`);}if(b?.seriesId){const oldScope=form.elements.applySeries?.closest('label');oldScope?.insertAdjacentHTML('afterend',`<fieldset class="bd-series-scope"><legend>Apply this edit to</legend><label><input type="radio" name="editScope" value="one" checked> Just this block</label><label><input type="radio" name="editScope" value="series"> This and all future blocks in the same plan</label></fieldset>`);oldScope?.remove();const deleteOne=form.querySelector('[data-delete-block]');deleteOne.textContent='Delete just this block';deleteOne.insertAdjacentHTML('afterend',`<button type="button" class="bd-danger" data-delete-series="${esc(b.seriesId)}" data-series-date="${esc(b.date)}">Delete this and future blocks</button>`);if(b.locked)form.querySelector('.bd-form-actions').insertAdjacentHTML('beforebegin',`<div class="bd-unlock-actions"><button type="button" class="bd-secondary" data-unlock-block="${esc(b.id)}">Unlock just this block</button><button type="button" class="bd-secondary" data-unlock-series="${esc(b.seriesId)}">Unlock every block in this recurring plan</button></div>`);}
    if(!b){const updateRepeat=()=>{form.querySelector('.bd-repeat-weekdays').hidden=form.elements.repeat.value!=='custom-days';form.querySelector('.bd-repeat-months').hidden=form.elements.repeat.value!=='yearly';};form.elements.repeat.addEventListener('change',updateRepeat);updateRepeat();if(form.elements.template)form.elements.template.addEventListener('change',()=>{const t=templates[Number(form.elements.template.value)];if(!t)return;for(const key of ['title','minutes','category','priority'])if(form.elements[key]&&t[key]!=null)form.elements[key].value=t[key];if(t.start!=null)form.elements.start.value=time(t.start);});}
    if(!b&&form.elements.template)form.elements.template.addEventListener('change',()=>{const t=templates[Number(form.elements.template.value)];if(!t)return;form.elements.repeat.value=t.repeat||'none';form.elements.repeat.dispatchEvent(new Event('change'));form.querySelectorAll('[name=repeatWeekday]').forEach(input=>input.checked=(t.repeatDays||[]).includes(Number(input.value)));form.querySelectorAll('[name=repeatMonth]').forEach(input=>input.checked=(t.months||[]).includes(Number(input.value)));});
    if(!b){for(const [value,label] of Object.entries({daily:'Daily',weekdays:'Monday–Friday','custom-days':'Choose weekdays',weekly:'Weekly'}))form.elements.repeat.querySelector(`[value="${value}"]`).textContent=label;form.elements.repeat.closest('label').insertAdjacentHTML('afterend','<label data-repeat-weeks>Repeat for how many weeks?<input name="repeatWeeks" type="number" min="1" max="52" step="1" value="4"></label>');const syncWeeks=()=>form.querySelector('[data-repeat-weeks]').hidden=!['daily','weekdays','custom-days','weekly'].includes(form.elements.repeat.value);form.elements.repeat.addEventListener('change',syncWeeks);syncWeeks();if(form.elements.template)form.elements.template.addEventListener('change',()=>{const t=templates[Number(form.elements.template.value)];if(t?.weeks)form.elements.repeatWeeks.value=t.weeks;});}
  }
  function noteDialog(id) { const n=ideas.find(n=>n.id===id);dialog(`<h2>A little thought.</h2><p>Ideas, reminders, a maybe. It all belongs here.</p><form id="bd-note-form" data-id="${esc(id||'')}"><label>Your note<textarea name="text" required rows="7" maxlength="10000">${esc(n?.text||'')}</textarea></label><label class="bd-check-label"><input type="checkbox" name="private" ${n?.private?'checked':''}> Hide this note until I reveal it</label><button class="bd-primary">Save note</button></form>`); }
  function moveBlockDialog(id){
    const block=blocks.find(item=>item.id===id);if(!block)return;const next=new Date(Math.max(Date.now(),fromKey(block.date).getTime()));next.setDate(next.getDate()+1);
    dialog(`<h2>Move this block.</h2><p>The original stays in your history as Moved. A new Planned block will be placed here.</p><form id="bd-move-form" data-id="${esc(id)}"><label>New day<input name="date" type="date" required value="${inputDate(dateKey(next))}"></label><label>New start time<input name="start" type="time" step="300" required value="${time(block.start)}"></label><button class="bd-primary">Move block</button></form>`);
  }
  document.addEventListener('submit',event=>{
    const form=event.target;if(!['bd-block-form','bd-note-form','bd-settings','bd-move-form'].includes(form.id))return;event.preventDefault();const values=new FormData(form);
    if(form.id==='bd-block-form') {
      const [h,m]=String(values.get('start')).split(':').map(Number),start=h+m/60,lengthMode=values.get('lengthMode');let duration=Number(values.get('minutes'))/60;
      if(lengthMode==='hours')duration=Number(values.get('hours'));if(lengthMode==='end'){const [endH,endM]=String(values.get('end')).split(':').map(Number);duration=endH+endM/60-start;}
      ...
      const status=old?.source==='google-calendar'?'external':(values.get('status')||blockStatus(old||{})),record={...old,id:id||uid(),title:values.get('title').trim(),start,duration,date,category:values.get('category'),priority:values.get('priority')||'normal',status:status==='external'?undefined:status,completed:status==='done',locked:values.has('locked')};
      if(id){if((values.get('editScope')==='series'||values.has('applySeries'))&&old?.seriesId){const pivot=dayNumber(old.date);blocks=blocks.map(item=>item.seriesId===old.seriesId&&dayNumber(item.date)>=pivot?{...item,title:record.title,start:record.start,duration:record.duration,category:record.category,priority:record.priority,status:record.status,completed:record.completed,locked:record.locked}:item);}else blocks=blocks.map(item=>item.id===id?record:item);}else {
        const repeat=values.get('repeat'),months=values.getAll('repeatMonth').map(Number),repeatDays=values.getAll('repeatWeekday').map(Number),repeatWeeks=Math.max(1,Math.min(52,Number(values.get('repeatWeeks'))||4)),seriesId=repeat!=='none'?uid():null;Object.assign(record,{recurring:repeat!=='none',recurrenceRule:repeat,seriesId,recurrenceDays:repeatDays,recurrenceMonths:months,recurrenceWeeks:repeatWeeks});blocks.push(record);
        if(['daily','weekdays','custom-days','weekly'].includes(repeat))for(let i=1,limit=repeatWeeks*7;i<limit;i++){const next=new Date(d);next.setDate(d.getDate()+i);if(repeat==='weekly'&&i%7!==0||repeat==='weekdays'&&[0,6].includes(next.getDay())||repeat==='custom-days'&&!repeatDays.includes(next.getDay()))continue;blocks.push({...record,id:uid(),date:dateKey(next),recurring:true});}
        if(repeat==='monthly')for(let i=1;i<12;i++){const next=new Date(d.getFullYear(),d.getMonth()+i,d.getDate(),12);if(next.getDate()!==d.getDate())continue;blocks.push({...record,id:uid(),date:dateKey(next),recurring:true});}
        if(repeat==='yearly')for(let year=d.getFullYear();year<=d.getFullYear()+2;year++)for(const chosenMonth of months){const next=new Date(year,chosenMonth,d.getDate(),12);if(next<=d||next.getMonth()!==chosenMonth)continue;blocks.push({...record,id:uid(),date:dateKey(next),recurring:true});}
      }
      if(values.has('saveTemplate')){const saved=read('bedo-block-templates',[]),template={id:uid(),title:record.title,start:record.start,minutes:Math.round(record.duration*60),category:record.category,priority:record.priority,repeat:record.recurrenceRule||'none',repeatDays:record.recurrenceDays||[],months:record.recurrenceMonths||[],weeks:record.recurrenceWeeks||4};const index=saved.findIndex(item=>item.title.toLowerCase()===template.title.toLowerCase());if(index>=0)saved[index]=template;else saved.unshift(template);write('bedo-block-templates',saved.slice(0,30));}
      selected=d;view='day';page='calendar';
    } else if(form.id==='bd-move-form') { const original=blocks.find(item=>item.id===form.dataset.id),[h,m]=String(values.get('start')).split(':').map(Number);if(!original||!values.get('date')||!Number.isFinite(h))return;blocks=blocks.map(item=>item.id===original.id?{...item,status:'moved',completed:false}:item);blocks.push({...original,id:uid(),date:values.get('date'),start:h+m/60,status:'planned',completed:false,recurring:false,seriesId:null,movedFrom:original.id});selected=fromKey(values.get('date'));view='day';page='calendar';
    } else if(form.id==='bd-note-form') { const text=values.get('text').trim();if(!text)return;const id=form.dataset.id,privateNote=values.has('private');if(id)ideas=ideas.map(n=>n.id===id?{...n,text,private:privateNote}:n);else ideas.unshift({id:uid(),text,private:privateNote,created:new Date().toISOString()}); }
  ...
  document.addEventListener('click',async event=>{
    ...
    if(b.dataset.complete){blocks=blocks.map(x=>x.id===b.dataset.complete&&isBedoBlock(x)?{...x,status:blockStatus(x)==='done'?'planned':'done',completed:blockStatus(x)!=='done'}:x);save();render();}
    if(b.dataset.reality){if(b.dataset.reality==='move')moveBlockDialog(b.dataset.id);else{blocks=blocks.map(item=>item.id===b.dataset.id&&isBedoBlock(item)?{...item,status:b.dataset.reality,completed:b.dataset.reality==='done'}:item);save();render();}}
    ...
    if(b.dataset.duplicateBlock){const source=blocks.find(item=>item.id===b.dataset.duplicateBlock);if(source){blockDialog(null,source.title);const form=document.getElementById('bd-block-form');form.elements.start.value=time(source.start);form.elements.minutes.value=Math.round(source.duration*60);form.elements.category.value=category(source);form.elements.priority.value=source.priority||'normal';form.elements.locked.checked=Boolean(source.locked);form.elements.date.focus();}}
    if(b.dataset.deleteSeries&&confirm('Delete this and all future blocks in the same recurring plan?')){const pivot=dayNumber(b.dataset.seriesDate);blocks=blocks.filter(item=>item.seriesId!==b.dataset.deleteSeries||dayNumber(item.date)<pivot);save();document.getElementById('bd-dialog')?.remove();render();}
    if(b.dataset.unlockBlock){blocks=blocks.map(item=>item.id===b.dataset.unlockBlock?{...item,locked:false}:item);save();document.getElementById('bd-dialog')?.remove();render();}
    if(b.dataset.unlockSeries){blocks=blocks.map(item=>item.seriesId===b.dataset.unlockSeries?{...item,locked:false}:item);save();document.getElementById('bd-dialog')?.remove();render();}
    ...
    if(b.dataset.revealNote){revealedNotes.add(b.dataset.revealNote);render();}
    ...
    if(action==='copy-live'){const field=document.getElementById('bd-live-link');try{await navigator.clipboard.writeText(field.value);b.textContent='Copied!';}catch{field.focus();field.select();b.textContent='Select & copy';}}
    if(action==='tour')startTour();
    if(action==='tips-toggle'){const panel=document.querySelector('[data-tips-panel]'),open=panel.hidden;panel.hidden=!open;b.setAttribute('aria-expanded',String(open));b.textContent=open?'Hide tips':'Show 3 tips';}
    ...
    if(action==='restore-autosave'){const snapshot=read('bedo-autosave-backups',[])[0]?.data;if(!snapshot){dialog('<h2>No autosave yet.</h2><p>BEDO creates recovery copies as you make changes.</p>');return;}if(confirm('Restore the latest autosave on this device? Your current device copy will be kept as another recovery point.')){window.BedoSync?.changed();for(const [field,key] of Object.entries({blocks:'bedo-blocks',ideas:'bedo-ideas',dailyNotes:'bedo-daily-notes',routines:'bedo-routines'}))localStorage.setItem(key,JSON.stringify(snapshot[field]||[]));const prefs=snapshot.settings?.find(item=>item.id==='preferences');if(prefs?.profile)localStorage.setItem('bedo-profile',JSON.stringify(prefs.profile));openWorkspace();}}
    ...
    if(action==='insights-toggle'){insightsExpanded=!insightsExpanded;render();}
    if(b.dataset.insightPeriod){insightPeriod=b.dataset.insightPeriod;render();}
    ...
    if(action==='prev'||action==='next'){const delta=action==='prev'?-1:1;if(view==='month')month=new Date(month.getFullYear(),month.getMonth()+delta,1);else selected.setDate(selected.getDate()+delta*(['week','checklist'].includes(view)?7:1));render();}
  });
  ...
  document.addEventListener('pointerdown',event=>{
    const handle=event.target.closest('[data-drag],[data-resize]');if(!handle||event.button!==0)return;
    const id=handle.dataset.drag||handle.dataset.resize,mode=handle.dataset.resize?'resize':'move',block=blocks.find(b=>b.id===id);if(!block||block.locked)return;const element=handle.closest('.bd-block'),track=element.parentElement;
    const timeline=track.parentElement;
    drag={mode,id:block.id,date:block.date,start:Number(block.start),duration:Number(block.duration),y:event.clientY,height:Number(track.dataset.hourHeight),element,handle,pointer:event.pointerId,moved:false,timeline,scrollStart:window.scrollY};handle.setPointerCapture(event.pointerId);element.classList.add('dragging');event.preventDefault();
  });
  document.addEventListener('pointermove',event=>{
    ...
    if(drag.mode==='resize'){drag.nextDuration=Math.max(5/60,Math.min(24-drag.start,Math.round((drag.duration+delta/drag.height)*12)/12));drag.element.style.height=Math.max(42,drag.nextDuration*drag.height-3)+'px';}else{drag.next=Math.max(0,Math.min(24-drag.duration,Math.round((drag.start+delta/drag.height)*12)/12));drag.element.style.top=drag.next*drag.height+'px';if(view==='week'){const target=[...root.querySelectorAll('.bd-week-grid [data-track-date]')].find(track=>{const rect=track.getBoundingClientRect();return event.clientX>=rect.left&&event.clientX<=rect.right&&event.clientY>=rect.top&&event.clientY<=rect.bottom;});root.querySelectorAll('.bd-week-grid [data-track-date]').forEach(track=>track.classList.toggle('drop-target',track===target));drag.nextDate=target?.dataset.trackDate||drag.date;}}drag.moved ||= Math.abs(delta)>3;
  });
  document.addEventListener('pointerup',event=>{
    if(!drag||drag.pointer!==event.pointerId)return;
    if(drag.moved)blocks=blocks.map(b=>b.id===drag.id&&b.date===drag.date?{...b,...(drag.mode==='resize'?{duration:drag.nextDuration}:{start:drag.next,date:drag.nextDate||drag.date})}:b);
  ...
  function tick(){
    ...
    if(minuteKey!==statusMinute){statusMinute=minuteKey;if(blocks.some(block=>isBedoBlock(block)&&blockStatus(block)==='planned'&&blockEnd(block)<=now)){render();return;}}
  ...
  function landing() {
    ...
    const footer=screen.querySelector('.bd-landing-footer');footer.insertAdjacentHTML('beforeend','<nav aria-label="Legal"><a href="/privacy.html">Privacy</a><a href="/terms.html">Terms</a></nav>');
    ...
    const bedoChoices=screen.querySelectorAll('.bd-bedo-types article'),choiceReplies=['You jump in, figure things out, and keep going.','You think it through, then make it happen.'];let bedoChoiceOpenedAt=0;
    bedoChoices.forEach((article,index)=>{const title=article.querySelector('h3').textContent;article.innerHTML=`<button type="button" class="bd-bedo-choice" aria-expanded="false">${esc(title)}</button><p class="bd-bedo-reply" hidden>${esc(choiceReplies[index])}</p>`;article.querySelector('button').addEventListener('click',()=>{const opening=article.querySelector('.bd-bedo-reply').hidden;bedoChoices.forEach(item=>{item.querySelector('.bd-bedo-reply').hidden=true;item.querySelector('button').setAttribute('aria-expanded','false');});if(opening){article.querySelector('.bd-bedo-reply').hidden=false;article.querySelector('button').setAttribute('aria-expanded','true');bedoChoiceOpenedAt=Date.now();}});});
    screen.querySelector('.bd-bedo-meaning').textContent='Brainstorm it. Envision it. Do it. Observe.';
    const dismissBedoChoice=()=>{if(Date.now()-bedoChoiceOpenedAt<200)return;bedoChoices.forEach(article=>{article.querySelector('.bd-bedo-reply').hidden=true;article.querySelector('button').setAttribute('aria-expanded','false');});};screen.addEventListener('scroll',dismissBedoChoice,{passive:true});
    screen.querySelector('.bd-hero>small').textContent='Try out the demo and see how it works.';
    const anywhere=document.createElement('section');anywhere.className='bd-anywhere';anywhere.innerHTML='<span class="bd-pill">NO APP STORE NEEDED</span><h2>Use BEDO anywhere.</h2><p>Open it in your browser or install it from the browser to your home screen. Your device copy stays available online or offline—even in airplane mode. Cloud saving and live sharing resume when you reconnect.</p><div><span>🌐 Use in your browser</span><span>📲 Install from the browser</span><span>✈️ Plan offline</span></div>';
    const intro=screen.querySelector('.bd-bedo-intro'),features=screen.querySelector('#features'),support=screen.querySelector('#support'),googleEntry=screen.querySelector('.bd-google-entry');intro.replaceWith(anywhere);support.before(intro);support.before(googleEntry);
    const tips=document.createElement('section');tips.className='bd-landing-tips';tips.innerHTML='<span class="bd-kicker">A GENTLER WAY TO PLAN</span><h2>Tips on timeblocking.</h2>'+timeblockingTips();features.after(tips);
  ...
  if(shared){
    ...
    const loadShared=()=>fetch((localStorage.getItem('bedo-sync-url')||window.BEDO_API_URL)+'?action=public&token='+encodeURIComponent(shared)+'&t='+Date.now()).then(r=>r.json()).then(r=>{
      ...
      const sharedDay=date=>{const dayItems=items.filter(b=>b.date===date),startHour=Math.max(0,Math.floor(Math.min(...dayItems.map(b=>Number(b.start)),8))-1),endHour=Math.min(24,Math.ceil(Math.max(...dayItems.map(b=>Number(b.start)+Number(b.duration)),17))+1),height=68;return `<section class="bd-shared-day"><h2>${dateLabel(fromKey(date),{weekday:'long',month:'long',day:'numeric',year:'numeric'})}</h2><div class="bd-shared-timeline" style="--shared-hour:${height}px;--shared-hours:${endHour-startHour}"><div class="bd-shared-axis">${Array.from({length:endHour-startHour},(_,i)=>`<span>${clockTime(startHour+i)}</span>`).join('')}</div><div class="bd-shared-track">${dayItems.map(b=>`<article class="bd-shared-block" style="top:${(Number(b.start)-startHour)*height}px;height:${Math.max(46,Number(b.duration)*height-3)}px;${blockColorStyle(category(b))}"><strong>${esc(b.title)}</strong><small>${clockTime(b.start)} – ${clockTime(Number(b.start)+Number(b.duration))} · ${esc(category(b))}</small>${b.completed?'<em>✓ Completed</em>':''}</article>`).join('')}</div></div></section>`;};
      root.innerHTML=`<main class="bd-shared"><header><span class="bd-logo">${logo()}</span><div><p class="bd-kicker">LIVE READ-ONLY SCHEDULE</p><h1>${esc(data.profile?.title||data.profile?.name||'Shared bedo')}</h1><p>${esc(data.from||'')} ${data.to&&data.to!==data.from?'– '+esc(data.to):''}</p></div></header>${dates.map(sharedDay).join('')||'<div class="bd-empty">No blocks are shared for these days.</div>'}<p class="bd-help">This live BEDO view refreshes automatically.</p></main>`;
