...
(function () {
  ...
  function blockDialog(id, text='') {
    ...
    const form=document.getElementById('bd-block-form'),actions=form.querySelector('.bd-form-actions');actions.insertAdjacentHTML('beforebegin',`<label class="bd-check-label"><input type="checkbox" name="locked" ${b?.locked?'checked':''}> Lock this block’s time</label>`);if(!b){const repeat=form.elements.repeat,blockDay=fromKey(day);repeat.insertAdjacentHTML('beforeend','<option value="monthly">Monthly · next 12 months</option><option value="yearly">Yearly · choose months</option>');actions.insertAdjacentHTML('beforebegin',`<fieldset class="bd-repeat-months" hidden><legend>Repeat in these months</legend>${Array.from({length:12},(_,i)=>`<label><input type="checkbox" name="repeatMonth" value="${i}" ${i===blockDay.getMonth()?'checked':''}>${new Date(2000,i,1).toLocaleDateString(undefined,{month:'short'})}</label>`).join('')}</fieldset>`);repeat.addEventListener('change',()=>form.querySelector('.bd-repeat-months').hidden=repeat.value!=='yearly');}
  }
  ...
  document.addEventListener('submit',event=>{
    ...
    if(form.id==='bd-block-form') {
      ...
      const record={...old,id:id||uid(),title:values.get('title').trim(),start,duration,date,category:values.get('category'),completed:old?.completed||false,locked:values.has('locked')};
      if(id) blocks=blocks.map(b=>b.id===id?record:b);else {
        const repeat=values.get('repeat'),months=values.getAll('repeatMonth').map(Number);record.recurring=repeat!=='none';record.recurrenceRule=repeat;blocks.push(record);
        if(['daily','weekdays','weekly'].includes(repeat))for(let i=1;i<28;i++){const next=new Date(d);next.setDate(d.getDate()+i);if(repeat==='weekly'&&i%7!==0||repeat==='weekdays'&&[0,6].includes(next.getDay()))continue;blocks.push({...record,id:uid(),date:dateKey(next),recurring:true});}
  ...
  document.addEventListener('pointerdown',event=>{
    ...
    const block=blocks.find(b=>b.id===handle.dataset.drag);if(!block||block.locked)return;const element=handle.closest('.bd-block'),track=element.parentElement;
