...
(function () {
  ...
  function realityPrompt(){
    const block=blocks.filter(item=>isBedoBlock(item)&&blockStatus(item)==='unconfirmed').sort((a,b)=>dayNumber(a.date)-dayNumber(b.date)||a.start-b.start)[0];
    if(!block)return '';
    return `<section class="bd-reality-prompt"><div><span>PLAN VS REALITY</span><strong>Did you do this?</strong><p>${esc(block.title)} · ${dateLabel(fromKey(block.date),{weekday:'short',month:'short',day:'numeric'})}, ${clockTime(block.start)}–${clockTime(Number(block.start)+Number(block.duration))}</p></div><div><button class="bd-primary" data-reality="done" data-id="${esc(block.id)}">Done</button><button class="bd-secondary" data-reality="move" data-id="${esc(block.id)}">Move</button><button class="bd-secondary" data-reality="skipped" data-id="${esc(block.id)}">Skip</button></div></section>`;
  }
