...
(function () {
  ...
  let view = !demo&&['month','day','week'].includes(rememberedView.view)?rememberedView.view:'month', selected = !demo&&rememberedView.date?fromKey(rememberedView.date):new Date(), month = !demo&&rememberedView.date?fromKey(rememberedView.date):new Date(), filter = 'All', drag = null;
  ...
  function render() {
    ...
    if(page==='calendar'){
      ...
      if(view==='week')layoutWeekOverlaps();
    }
  ...
  function timeblockingTips(){return `<div class="bd-tips-grid"><article><span>BEDO’S RHYTHM</span><h3>Plan, adjust, notice.</h3><p>Plan the week before it starts. Adjust tomorrow the night before. Check blocks at midday and before bed—not for perfection, but awareness.</p><small>By BEDO</small></article><article><span>MAKE THE WHEN SPECIFIC</span><h3>Give intentions a time.</h3><p>Decide when and how you will act. A specific plan makes an intention easier to begin.</p><a href="https://doi.org/10.1016/S0065-2601(06)38002-1" target="_blank" rel="noopener">By Gollwitzer & Sheeran · Implementation Intentions research ↗</a></article><article><span>PLAN FOR REAL LIFE</span><h3>Leave breathing room.</h3><p>Expect interruptions and keep some buffer. A useful schedule can bend when the day changes.</p><a href="https://pubmed.ncbi.nlm.nih.gov/29154579/" target="_blank" rel="noopener">By Parke et al. · Journal of Applied Psychology ↗</a></article></div>`;}
  function savingCard() {
  ...
  document.addEventListener('submit',event=>{
    ...
    if(form.id==='bd-block-form') {
      ...
      const record={...old,id:id||uid(),title:values.get('title').trim(),start,duration,date,category:values.get('category'),completed:values.get('status')==='completed',locked:values.has('locked')};
      if(id){if((values.get('editScope')==='series'||values.has('applySeries'))&&old?.seriesId){const pivot=dayNumber(old.date);blocks=blocks.map(item=>item.seriesId===old.seriesId&&dayNumber(item.date)>=pivot?{...item,title:record.title,start:record.start,duration:record.duration,category:record.category,completed:record.completed,locked:record.locked}:item);}else blocks=blocks.map(item=>item.id===id?record:item);}else {
        const repeat=values.get('repeat'),months=values.getAll('repeatMonth').map(Number),repeatDays=values.getAll('repeatWeekday').map(Number),seriesId=repeat!=='none'?uid():null;Object.assign(record,{recurring:repeat!=='none',recurrenceRule:repeat,seriesId,recurrenceDays:repeatDays,recurrenceMonths:months});blocks.push(record);
        if(['daily','weekdays','custom-days','weekly'].includes(repeat))for(let i=1,limit=repeat==='daily'?28:84;i<limit;i++){const next=new Date(d);next.setDate(d.getDate()+i);if(repeat==='weekly'&&i%7!==0||repeat==='weekdays'&&[0,6].includes(next.getDay())||repeat==='custom-days'&&!repeatDays.includes(next.getDay()))continue;blocks.push({...record,id:uid(),date:dateKey(next),recurring:true});}
      ...
      if(values.has('saveTemplate')){const saved=read('bedo-block-templates',[]),template={id:uid(),title:record.title,start:record.start,minutes:Math.round(record.duration*60),category:record.category,repeat:record.recurrenceRule||'none',repeatDays:record.recurrenceDays||[],months:record.recurrenceMonths||[]};const index=saved.findIndex(item=>item.title.toLowerCase()===template.title.toLowerCase());if(index>=0)saved[index]=template;else saved.unshift(template);write('bedo-block-templates',saved.slice(0,30));}
