...
(function () {
  ...
  function render() {
    ...
    if(page==='calendar'){
      ...
      if(view==='week'){layoutWeekOverlaps();requestAnimationFrame(layoutWeekOverlaps);}
    }
  ...
  document.addEventListener('submit',event=>{
    ...
    if(form.id==='bd-block-form') {
      ...
      const record={...old,id:id||uid(),title:values.get('title').trim(),start,duration,date,category:values.get('category'),priority:values.get('priority')||'normal',completed:values.get('status')==='completed',locked:values.has('locked')};
      if(id){if((values.get('editScope')==='series'||values.has('applySeries'))&&old?.seriesId){const pivot=dayNumber(old.date);blocks=blocks.map(item=>item.seriesId===old.seriesId&&dayNumber(item.date)>=pivot?{...item,title:record.title,start:record.start,duration:record.duration,category:record.category,priority:record.priority,completed:record.completed,locked:record.locked}:item);}else blocks=blocks.map(item=>item.id===id?record:item);}else {
  ...
  document.addEventListener('click',async event=>{
    ...
    if(b.dataset.unlockSeries){const pivot=dayNumber(b.dataset.seriesDate);blocks=blocks.map(item=>item.seriesId===b.dataset.unlockSeries&&dayNumber(item.date)>=pivot?{...item,locked:false}:item);save();document.getElementById('bd-dialog')?.remove();render();}
