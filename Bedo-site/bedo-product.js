(function () {
  ...
  const profile = () => read("bedo-profile", {});
  const apiUrl = () => localStorage.getItem("bedo-sync-url") || API_URL;
  const session = () => localStorage.getItem("bedo-auth-session") || "";
  ...
  function mountProfileSettings() {
    ...
    if (!content || document.getElementById("bedo-profile-settings")) return;
    const user = read("bedo-auth-user", {}), saved = profile(), section = document.createElement("section");
    section.id = "bedo-profile-settings"; section.className = "setting-section";
    section.innerHTML = '<h2>Your Bedo</h2><p>Choose the identity and color shown in your planner and shared schedule.</p><div class="profile-grid"><label>Display name<input class="input" data-profile-name maxlength="40" value="' + esc(saved.name || user.name || "") + '"></label><label>Planner title<input class="input" data-profile-title maxlength="60" value="' + esc(saved.title || "My Bedo") + '"></label></div><div class="palette-picker">' + themes.map(theme => '<button type="button" class="palette ' + theme + '" data-palette="' + theme + '" aria-label="Use ' + theme + ' theme"></button>').join("") + '</div><div class="profile-actions"><button class="button primary" type="button" data-save-profile>Save identity</button><button class="button" type="button" data-switch-account>Switch Google account</button><button class="button ghost danger" type="button" data-sign-out>Sign out</button></div>';
  ...
  function shareDialog() {
    document.getElementById("bedo-share-dialog")?.remove();
    const saved = read("bedo-share", {}), dialog = document.createElement("div");
    dialog.className = "overlay"; dialog.id = "bedo-share-dialog";
  ...
  async function publishShare() {
    ...
    const blocks = read("bedo-blocks", []).map(({ id, title, start, duration, date, color }) => ({ id, title, start, duration, date, color }));
    const notes = document.querySelector("[data-share-notes]")?.checked ? read("bedo-daily-notes", []) : [];
    try {
      ...
      localStorage.setItem("bedo-share", JSON.stringify({ token: result.token, url, includeNotes: Boolean(notes.length) }));
  ...
  async function unpublishShare() {
    const saved = read("bedo-share", {}), status = document.querySelector("[data-share-status]");
    try {
      ...
      localStorage.removeItem("bedo-share"); status.textContent = "The shared link has been disabled.";
    } catch (error) { status.textContent = error.message; }
  }
  ...
  async function renderPublicSchedule(token) {
    document.documentElement.classList.add("public-view"); document.body.innerHTML = '<main class="public-schedule"><p>Loading shared Bedo…</p></main>';
    try {
      ...
      const data = result.data || {}, owner = data.profile?.name || "A Bedo user";
  ...
  function mountTopbar() {
    ...
    if (!document.getElementById("bedo-top-logo")) {
      const logo = document.createElement("a"); logo.id = "bedo-top-logo"; logo.className = "theme-logo"; logo.href = "/"; logo.setAttribute("aria-label", "Bedo home"); left.prepend(logo);
    }
    if (!document.getElementById("bedo-clock")) {
      const clock = document.createElement("time"); clock.id = "bedo-clock"; clock.className = "device-clock"; actions.prepend(clock);
    ...
    if (new URLSearchParams(location.search).has("demo") && !document.getElementById("bedo-exit-demo")) {
      const exit = document.createElement("button"); exit.id = "bedo-exit-demo"; exit.className = "button demo-exit"; exit.type = "button"; exit.textContent = "← Exit demo"; exit.addEventListener("click", () => window.BedoDemo?.exit()); actions.prepend(exit);
    }
  }
  function mountGlobalQuickNote() {
    if (document.getElementById("bedo-global-note")) return;
    const button = document.createElement("button");
    button.id = "bedo-global-note";
    ...
    let saved = read("bedo-quick-note-position", null);
    ...
    const finish = event => {
      ...
      localStorage.setItem("bedo-quick-note-position", JSON.stringify({ x: button.offsetLeft, y: button.offsetTop }));
      drag = null;
    };
  ...
  document.addEventListener("click", event => {
    ...
    if (event.target.closest("[data-close-share]")) document.getElementById("bedo-share-dialog")?.remove();
    ...
    if (palette) {
      ...
      localStorage.setItem("bedo-profile", JSON.stringify({ ...profile(), theme: selectedTheme }));
    }
    if (event.target.closest("[data-save-profile]")) { localStorage.setItem("bedo-profile", JSON.stringify({ name: document.querySelector("[data-profile-name]").value.trim(), title: document.querySelector("[data-profile-title]").value.trim(), theme: document.documentElement.dataset.palette || "sage" })); applyIdentity(); }
    if (event.target.closest("[data-sign-out]")) window.BedoAuth?.signOut(); if (event.target.closest("[data-switch-account]")) window.BedoAuth?.switchAccount();
  });
