...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.evaluate(()=>window.drivePromptCount||0),0,'opening BEDO does not automatically request Google Drive again');await page.evaluate(()=>window.BedoSync.connect());
  ...
  assert.equal(await page.getByRole('link',{name:'Sign in with Google'}).count(),0,'signed-in settings do not ask the user to sign in again');
  await page.goto(base+'/login',{waitUntil:'domcontentloaded'});await page.waitForURL('**/?app=1');
  assert.equal(await page.getByText('Test Google login',{exact:true}).count(),0,'a remembered account bypasses the login screen');
  await page.locator('[data-page=calendar]:visible').click();await page.locator('[data-view=checklist]').click();const returning=await context.newPage();await returning.goto(base+'/',{waitUntil:'domcontentloaded'});await returning.waitForURL('**/?app=1');await returning.locator('[data-view=checklist].active').waitFor();assert.equal(await returning.evaluate(()=>window.drivePromptCount||0),0,'closing or reopening the app does not trigger a Drive login popup');await returning.close();
  assert.deepEqual(errors,[]);
  await context.close();
  const localContext=await browser.newContext({serviceWorkers:'block'}),local=await localContext.newPage(),localErrors=[];local.on('pageerror',error=>localErrors.push(error.message));
  await localContext.route('**/auth-config.js',r=>r.fulfill({contentType:'application/javascript',body:'window.BEDO_GOOGLE_CLIENT_ID="";'}));
  await local.goto(base+'/?app=1',{waitUntil:'domcontentloaded'});await local.evaluate(()=>{const d=new Date(),date=`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`;localStorage.setItem('bedo-auth-user',JSON.stringify({sub:'local'}));localStorage.setItem('bedo-auth-session','session');localStorage.setItem('bedo-calendar-choice','fresh');localStorage.setItem('bedo-blocks',JSON.stringify([{id:'keep-private',title:'Keep private',date,start:23,duration:.5,category:'Personal'}]));});await local.reload({waitUntil:'domcontentloaded'});
  await local.locator('.bd-shell').waitFor();assert.deepEqual(localErrors,[]);assert.equal(await local.evaluate(()=>JSON.parse(localStorage.getItem('bedo-blocks'))[0].id),'keep-private');
