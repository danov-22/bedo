...
(async()=>{
 ...
 try{
  ...
  await context.route('**/auth-config.js',r=>r.fulfill({contentType:'application/javascript',body:`window.BLOCKDAY_GOOGLE_CLIENT_ID=${JSON.stringify(client)};`}));
  ...
  assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('blockday-ideas'))[0].id),'cloud-note','Google login restores notes before first tour');
  ...
  assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('blockday-tour-state')).completed),true);
  ...
  await page.waitForFunction(()=>window.BlockdaySync.getStatus().state==='saved');
  ...
  await page.waitForFunction(()=>window.BlockdaySync.getStatus().state==='error');
  assert.equal(await page.evaluate(()=>localStorage.getItem('blockday-sync-pending')),'true');
  assert(await page.evaluate(()=>JSON.parse(localStorage.getItem('blockday-ideas')).some(n=>n.text==='Keep on device if save fails')));
  failSave=false;await page.locator('[data-action=sync-now]').click();await page.waitForFunction(()=>window.BlockdaySync.getStatus().state==='saved');
  ...
  await bad.goto(base+'/',{waitUntil:'domcontentloaded'});await bad.evaluate(()=>{localStorage.setItem('blockday-auth-user',JSON.stringify({sub:'failure-test'}));localStorage.setItem('blockday-auth-session','failure-session');localStorage.setItem('blockday-blocks',JSON.stringify([{id:'keep-private'}]));});
  await bad.goto(base+'/?app=1',{waitUntil:'domcontentloaded'});await bad.locator('#bd-retry-workspace').waitFor();
  assert.equal(failedSaves,0,'failed initial load never uploads an empty plan');assert.equal(await bad.evaluate(()=>JSON.parse(localStorage.getItem('blockday-blocks'))[0].id),'keep-private');
