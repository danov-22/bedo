...
(async()=>{
 ...
 try{
  ...
  await context.route(base+'/login',async r=>{const response=await context.request.get(base+'/');await r.fulfill({status:200,contentType:'text/html',body:await response.text()});});
