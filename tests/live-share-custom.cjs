...
(async()=>{
 ...
 await page.locator('[data-action=close]').click();await page.locator('[data-page=calendar]:visible').click();await page.locator('[data-view=month]').click();await page.locator(`.bd-month-day[data-date="${today}"]`).first().click();await page.locator('[data-edit=visible]').click();await page.locator('[name=title]').fill('Live update');await page.locator('#bd-block-form .bd-primary').click();
 for(let i=0;i<50&&publicData?.blocks?.[0]?.title!=='Live update';i++)await new Promise(resolve=>setTimeout(resolve,100));
