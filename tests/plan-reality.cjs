...
(async()=>{
 ...
 assert(await page.getByText('UP NEXT').isVisible(),'main app shows the Up next card');assert(await page.locator('.bd-reality-prompt').getByText('Past BEDO block',{exact:true}).isVisible(),'block plan is prominent');
