...
(async()=>{
 ...
 let records=await page.evaluate(()=>JSON.parse(localStorage.getItem('bedo-blocks')).filter(b=>b.title==='Monthly review'));assert(records.length>=11&&records.length<=12,'monthly repeat creates up to twelve valid monthly dates; found '+records.length+'; JS: '+errors.join('|'));assert(records.every(b=>b.locked));
