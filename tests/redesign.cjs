    ...
    for (const [name,width,height] of [['mobile',390,844],['tablet',820,1180],['desktop',1440,1000]]) {
      ...
      for(const mode of ['light','dark']){
        ...
        for(const palette of ['sage','ocean','berry','neutral']){
          ...
          if(palette==='neutral')assert.equal(colors.logoBg,mode==='dark'?'rgb(70, 75, 87)':'rgb(255, 255, 255)','neutral logo follows theme, not green');
          if(mode==='dark') {
      ...
      assert.equal(await page.locator('.bd-month-day.today .bd-day-incomplete').textContent(),'🎗️');
