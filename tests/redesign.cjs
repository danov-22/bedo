...
(async () => {
  ...
  try {
    for (const [name,width,height] of [['mobile',390,844],['tablet',820,1180],['desktop',1440,1000]]) {
      ...
      assert.equal(await page.locator('.bd-preview .peach').evaluate(el=>getComputedStyle(el).color),'rgb(48, 40, 0)','yellow preview block keeps readable contrasting text');
      assert.deepEqual(await page.locator('.bd-preview-plan article').evaluateAll(els=>els.map(el=>getComputedStyle(el).backgroundColor)),['rgb(112, 66, 212)','rgb(242, 204, 33)','rgb(0, 132, 61)'],'landing preview uses the same solid block colors as the app');
