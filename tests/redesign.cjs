...
(async () => {
  ...
  try {
    for (const [name,width,height] of [['mobile',390,844],['tablet',820,1180],['desktop',1440,1000]]) {
      ...
      await page.screenshot({path:path.join(process.env.TEMP,`blockday-calendar-status-${name}.png`)});
      ...
      for(const mode of ['light','dark']){
        ...
        for(const hex of ['#7c5ce7','#ffffff','#000000','#ffff00']){
          ...
          if(hex==='#7c5ce7')await page.screenshot({path:path.join(process.env.TEMP,`blockday-custom-${name}-${mode}.png`)});
        }
      }
      ...
      assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('blockday-profile')).customColor),'#ffff00','invalid input does not overwrite saved color');
    ...
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('blockday-profile')).customColor),'#336699','saving preferences also saves custom color');
    await page.reload({waitUntil:'domcontentloaded'});
    assert.equal(await page.evaluate(()=>document.documentElement.dataset.palette),'custom');
