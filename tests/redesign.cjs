...
(async () => {
  ...
  try {
    for (const [name,width,height] of [['mobile',390,844],['tablet',820,1180],['desktop',1440,1000]]) {
      ...
      assert.equal(await page.locator('.bd-bedo-meaning').textContent(),'Brainstorm it. Envision it. Do it. Observe.');
      const choices=page.locator('.bd-bedo-choice');assert.equal(await choices.count(),2);
      await choices.nth(0).click();assert.equal(await page.locator('.bd-bedo-reply:visible').textContent(),'You jump in, figure things out, and keep going.');
      await choices.nth(1).click();assert.equal(await page.locator('.bd-bedo-reply:visible').textContent(),'You think it through, then make it happen.');
      await page.waitForTimeout(220);await page.locator('.bd-landing').evaluate(el=>el.scrollTop+=10);await page.waitForTimeout(50);assert.equal(await page.locator('.bd-bedo-reply:visible').count(),0,'choice response closes when the landing page scrolls');
      ...
      const supportContrast=await page.locator('.bd-support').evaluate(el=>{const section=getComputedStyle(el),message=getComputedStyle(el.querySelector('small'));return {background:section.backgroundColor,text:section.color,message:message.color};});
      assert.notEqual(supportContrast.background,supportContrast.text,'support text contrasts with its dark background');assert.equal(supportContrast.message,supportContrast.text,'thank-you message remains readable in dark mode');
      await page.evaluate(()=>localStorage.setItem('bedo-theme',JSON.stringify('light')));await page.reload({waitUntil:'domcontentloaded'});
      ...
      assert(supplied.includes('fill="#165bdf"'),'website SVG logo uses the blue brand color');
      ...
      assert((await page.locator('.bd-anywhere').textContent()).includes('airplane mode'));
      assert.equal(await page.locator('.bd-hero>small').textContent(),'Try out the demo and see how it works.');
      assert(Number.parseFloat(await page.locator('.bd-landing-footer span').evaluate(el=>getComputedStyle(el).fontSize))>=12);
      assert.equal(await page.locator('.bd-preview .peach').evaluate(el=>getComputedStyle(el).color),await page.locator('.bd-preview').evaluate(el=>getComputedStyle(el).color),'preview text follows the readable page contrast color');
      assert(await page.locator('.bd-preview-plan article').first().evaluate(el=>{const color=getComputedStyle(el).backgroundColor;return color.includes('/')||color.startsWith('rgba');}),'landing preview blocks use transparent color fills');
      ...
      const handle=page.locator('[data-drag="demo-1"]');await handle.evaluate(el=>window.scrollTo(0,scrollY+el.getBoundingClientRect().top-innerHeight/2));const box=await handle.boundingBox();
      ...
      if(name==='mobile'){
        await page.locator('[data-drag="demo-1"]').evaluate(el=>window.scrollTo(0,scrollY+el.getBoundingClientRect().top-innerHeight/2));const touch=await context.newCDPSession(page),target=await page.locator('[data-drag="demo-1"]').boundingBox();
      ...
      assert.deepEqual(await page.locator('.bd-palettes [data-palette]').evaluateAll(els=>els.map(el=>el.dataset.palette)),['ocean','berry','sage','neutral']);
