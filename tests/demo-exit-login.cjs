...
(async()=>{
 ...
 await page.evaluate(()=>{localStorage.setItem('bedo-auth-user',JSON.stringify({sub:'returning-user',email:'returning@example.test'}));localStorage.setItem('bedo-auth-session','session');localStorage.setItem('bedo-calendar-choice','fresh');localStorage.setItem('bedo-tour-state',JSON.stringify({completed:true}));});
